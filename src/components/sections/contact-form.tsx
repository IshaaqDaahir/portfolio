...
export default function ContactForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(form.action || `https://formspree.io/f/${process.env.FORMSPREE_ID}`, {
  ...
  return (
    <>
      <form action={`https://formspree.io/f/${process.env.FORMSPREE_ID}`} method="POST" className="space-y-4" onSubmit={handleSubmit}>
